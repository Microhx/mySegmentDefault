...
public class UserCommentModelImpl implements UserCommentContract.UserCommentModel {
    ...
    @Override
    public Observable<UserCommentResultDataEntity> addUserComment(int enterType,String newsId, boolean isReply, String text, String token) {
        if(isReply) {
        ...
        return Api.getApiService(0).addUserNewsComment(getEnterType(enterType),newsId,text,token).compose(RxSchedulers.<UserCommentResultDataEntity>io_main());
    }

    private String getEnterType(int enterType) {
        if(enterType == 0) return "news";
        return "note";
    }
}
